...
- 데이터: Home Credit Default Risk (Kaggle 2018). `application_train` 307,511행 × 122열 + `bureau.csv` 1,716,428행
- 모델: LightGBM AUC 0.7605(application only) → 0.7663(+bureau, 최종 채택). XGBoost가 0.7667로 근소 우위였으나 해석 용이성을 위해 LightGBM 채택
- 접근: bureau 데이터 조인·집계, PSM 인과추론, 승인 임계값별 비즈니스 임팩트 시뮬레이션, SHAP 해석, selection bias 진단
- 목표: 모델 정확도에 그치지 않고 누구를 어떤 이유로 승인·거절할지 설명 가능한 신용리스크 분석
